...
from weatherwear.domain.types import CityResolutionResult, ExecutionRecord, LocationCandidate
from weatherwear.support.city_aliases import COMMON_CITY_ALIASES
from weatherwear.support.common_utils import (
...
def _message(locale: str, key: str) -> str:
    messages = {
        "strict_confirm": {
            "zh-CN": "严格确认模式已开启，请确认你想查询的候选城市。",
            "en-US": "Strict confirmation is enabled. Please confirm the location candidate.",
        },
        "same_city_ambiguous": {
            "zh-CN": "检测到多个同名城市候选，请确认你想查询的是哪一个。",
            "en-US": "Multiple cities share the same name. Please confirm which one you want.",
        },
        "close_candidates": {
            "zh-CN": "检测到多个接近的城市候选，请确认你想查询的是哪一个。",
            "en-US": "Several close candidates were found. Please confirm the final location.",
        },
        "low_confidence": {
            "zh-CN": "城市匹配置信度偏低，建议补充国家、州省或更完整的地名。",
            "en-US": "Location confidence is low. Add a country, region, or a more specific place name.",
        },
    }
    locale_key = "en-US" if _is_english(locale) else "zh-CN"
    return messages[key][locale_key]
...
def _cache_key(
    ...
    return "||".join(
        [
            ...
            confirmation_mode,
            "en" if _is_english(locale) else "zh",
...
def _choose_best_candidate(
    ...
    if confirmation_mode == "strict":
        _mark_clarification(result, candidates, _message(locale, "strict_confirm"))
        return result
    ...
    if (
        len(candidates) == 1
        and top.metadata.get("degraded_mode")
        and not has_explicit_region_hint(result.normalized_input)
    ):
        result.selected = top
    ...
    if len(candidates) > 1:
        ...
        if same_city_ambiguous and (top.confidence - second.confidence) <= 0.14:
            _mark_clarification(result, top_group, _message(locale, "same_city_ambiguous"))
            return result
        if not has_explicit_region_hint(result.normalized_input) and (top.confidence - second.confidence) <= 0.08:
            _mark_clarification(result, candidates, _message(locale, "close_candidates"))
            return result

    if top.confidence < 0.58:
        _mark_clarification(result, candidates, _message(locale, "low_confidence"))
        return result
...
def _build_cache_record(normalized_input: str, resolution_status: str) -> ExecutionRecord:
    return ExecutionRecord(
        ...
        used_llm=False,
        fallback_used=False,
        metadata={"cache_hit": True, "cached_resolution_status": resolution_status},
    )
...
def _attempt_fast_resolution(
    ...
    if "alias_lookup" in fallback_policy:
        alias_started_at = time.time()
        alias_seed = _lookup_alias(normalize_text(normalized_input))
        if alias_seed:
            result.used_alias = True
            seed_candidate = _candidate_from_seed(alias_seed, normalized_input, "alias_seed", 0.97)
            ...
            result.execution_records.append(
                ExecutionRecord(
                    ...
                    name="AliasLookup",
                    node_name="resolve_city",
                    step_kind="alias_lookup",
                    provider="alias_dictionary",
                    success=True,
                    used_llm=False,
                    fallback_used=False,
                    elapsed_ms=int((time.time() - alias_started_at) * 1000),
                    input_summary=normalized_input,
                    output_summary=seed_candidate.display_name,
                    metadata={"matched_alias": True, "candidate_id": seed_candidate.candidate_id},
                )
            )
    ...
    for candidate in seed_candidates:
        if cancel_token and hasattr(cancel_token, "raise_if_cancelled"):
            cancel_token.raise_if_cancelled("resolve_city:fast_validate")
        validated_candidates.extend(
            _call_with_supported_kwargs(
                weather_service.validate_candidate,
                candidate,
    ...
    if seed_candidates:
        result.execution_records.append(
            ExecutionRecord(
                ...
                node_name="resolve_city",
                step_kind="candidate_validation",
                provider="openweather_geocoding",
                success=bool(validated_candidates),
                used_llm=False,
                fallback_used=False,
                elapsed_ms=int((time.time() - validate_started_at) * 1000),
                input_summary=f"seed_candidates={len(seed_candidates)}",
                output_summary=f"validated={len(validated_candidates)}",
                metadata={"input_count": len(seed_candidates), "validated_count": len(validated_candidates)},
            )
        )

    if "direct_geocoding" in fallback_policy:
        if cancel_token and hasattr(cancel_token, "raise_if_cancelled"):
            cancel_token.raise_if_cancelled("resolve_city:fast_direct_geocode")
        direct_started_at = time.time()
        direct_hits = _call_with_supported_kwargs(
            weather_service.geocode_city,
            normalized_input,
            limit=6,
            cancel_token=cancel_token,
        )
        ...
        result.execution_records.append(
            ExecutionRecord(
                role="Direct geocoding",
                name="DirectGeocoding",
                node_name="resolve_city",
                step_kind="direct_geocoding",
                provider="openweather_geocoding",
                success=bool(direct_hits),
                used_llm=False,
                fallback_used=bool(direct_hits),
                elapsed_ms=int((time.time() - direct_started_at) * 1000),
                input_summary=normalized_input,
                output_summary=f"hits={len(direct_hits)}",
                metadata={"hits": len(direct_hits), "fallback_used": bool(direct_hits)},
            )
        )

    result.validated_candidates = _unique_validated_candidates(validated_candidates)
    return _choose_best_candidate(
        result,
...
def resolve_city(
    ...
    *,
    ...
    cache_key = _cache_key(raw_query, fallback_policy, fast_mode, confirmation_mode, locale)
    ...
    if fast_mode:
        if cancel_token and hasattr(cancel_token, "raise_if_cancelled"):
            cancel_token.raise_if_cancelled("resolve_city:fast_mode")
    ...
    if should_use_llm:
        if cancel_token and hasattr(cancel_token, "raise_if_cancelled"):
            cancel_token.raise_if_cancelled("resolve_city:llm_before")
        prompt = (
        ...
        record.step_kind = "llm_city_resolution"
    ...
    for candidate in merged_candidates:
        if cancel_token and hasattr(cancel_token, "raise_if_cancelled"):
            cancel_token.raise_if_cancelled("resolve_city:validate_candidate")
        validated_candidates.extend(
            _call_with_supported_kwargs(
                weather_service.validate_candidate,
                candidate,
    ...
    result.execution_records.append(
        ExecutionRecord(
            ...
            fallback_used=False,
            elapsed_ms=int((time.time() - validate_started_at) * 1000),
            input_summary=f"merged_candidates={len(merged_candidates)}",
            output_summary=f"validated={len(validated_candidates)}",
            metadata={"input_count": len(merged_candidates), "validated_count": len(validated_candidates)},
        )
    )

    if not validated_candidates and "direct_geocoding" in fallback_policy:
        if cancel_token and hasattr(cancel_token, "raise_if_cancelled"):
            cancel_token.raise_if_cancelled("resolve_city:direct_geocode")
        direct_started_at = time.time()
        direct_hits = _call_with_supported_kwargs(
            weather_service.geocode_city,
            normalized_input,
    ...
    result.validated_candidates = _unique_validated_candidates(validated_candidates)
    final_result = _choose_best_candidate(
        result,
        preferred_candidate_id=preferred_candidate_id,
        confirmation_mode=confirmation_mode,
        locale=locale,
