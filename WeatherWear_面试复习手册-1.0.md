...
WeatherWear 里有两层“工具能力”概念：一层是项目内部通过 coordinator 编排的 service 调用，另一层是用 FastMCP 暴露出来的可选天气工具接口。主链路属于 code-orchestrated tool call，而不是模型原生 function calling；MCP 是额外的协议化扩展能力，不是主链路依赖。
...
也就是说，地图能力增强的是 location resolution，而不是多模态理解。

### 7.4 面试表达

WeatherWear 支持文本查询和地图坐标查询两种入口，地图能力主要服务于地点解析、候选确认和坐标直查天气。它并没有接入图片、语音或视觉推理模型，因此不能把它表述成多模态大模型系统，更准确的说法是“带地图交互的单场景 LLM 产品”。
...
history 和 favorites 是用户状态持久化，不是语义记忆。它们不会被自动抽取成长期画像，也不会作为跨会话的知识记忆回注到 prompt 中，所以不能把它们说成长期记忆，只能说是 runtime persistence。
...
## 12.8 前端开发者页面体现了哪些工程能力

它体现的是可观测性和可维护性。开发者页面能直接看到 trace、timeline、runtime health、model config、map config、logs 和 dependency status，让问题可以被解释、被定位、被复现，而不是只看最终回答对不对。
...
> WeatherWear 是一个面向天气穿搭场景的 LLM 应用工程项目。它不是通用 Agent 平台，而是用 coordinator + 状态图 workflow 把地点解析、天气查询、轻量 RAG 和穿搭建议生成串起来，同时做了完整的降级链和开发者可观测页面。它的亮点在于 workflow 编排、可解释检索、运行稳定性和工程完整度，而不是开放式自治能力。
...
1. 先讲定位  
这是一个单场景 LLM 工作流产品，目标是把天气穿搭这件事做成完整工程，而不是做通用 agent。

2. 再讲主链路  
用户输入文本或地图坐标后，系统会先做场景上下文提取，再决定 fast path 还是 supervisor planner，然后完成城市解析、天气获取、轻量 RAG、穿搭生成和结果 view model 组装。

3. 再讲技术亮点  
核心亮点是轻量 RAG、LangGraph 兼容执行、完整降级链和开发者可观测性。embedding、Chroma、OpenWeather、LLM 任意一层出问题，系统都尽量退到还能工作的路径。
